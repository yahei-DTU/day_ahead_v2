...
MODELS=(lightgbm lightgbm_unbalanced)
EXPERIMENTS=(sample_weighting no_sample_weighting)

IDX=$((LSB_JOBINDEX - 1))
MODEL=${MODELS[$((IDX / 2))]}
EXPERIMENT=${EXPERIMENTS[$((IDX % 2))]}
...
python -m day_ahead_v2.train --config-name=config_test experiments=$EXPERIMENT models=$MODEL
